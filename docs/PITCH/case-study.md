# 📜 Case Study — The First Summoning

**European Energy Software Market — A Live Demonstration of Solstein**

> *"We ran the spell. Here is what it produced."*

*Status: Complete — Available for walkthrough in any introductory session*

---

## The Brief

A PE firm needed to understand the European energy software and utilities management market before making acquisition decisions. They needed to know who was winning, who was fading, and where the hidden opportunities lived.

The traditional path: commission a consulting firm, wait three months, receive a report covering 8–10 companies.

**We ran Solstein instead.**

| Parameter | Value |
|-----------|-------|
| Market | European Energy Software & Utilities Management |
| Geography | Netherlands, Germany, Belgium, UK, Spain, Nordics |
| Companies entered into the ritual | 29 |
| Days to complete | **3** |
| Refresh cadence | Continuous — the data does not go stale |

---

## What the Spell Revealed

### The Attractiveness Board

Every company was scored across three dimensions:

| Dimension | What Solstein Read |
|-----------|-------------------|
| **Growth Score (0–10)** | Revenue trajectory, growth rate, margin health and direction |
| **Financial Health Score (0–10)** | Revenue scale against sector baseline, funding cushion, operational efficiency |
| **Competitive Position Score (0–10)** | AI adoption maturity, SaaS transition progress, tech stack depth, protocol certifications, geographic footprint |

### The Classification Verdict

| Classification | Count | Share of Market | What It Signals |
|---------------|-------|-----------------|-----------------|
| Rocket | 4 | 14% | High-growth, AI-native or rapidly adopting. Acquisition premium justified now. |
| Neutral | 18 | 62% | Stable players. Signal-rich for anyone reading carefully. |
| Dinosaur | 7 | 24% | Carrying coal. Three have transformation potential. Four are irreversible. Solstein shows you which is which. |

### What Else the Crystal Revealed

- **15+ European energy protocols** mapped to exactly which companies hold certifications — exposing gaps competitors do not advertise
- **M&A genealogy** for 8 major players — full acquisition family trees showing who absorbed what and which synergies were claimed vs. realized
- **Country-by-country presence matrix** for all 29 companies — not headquarters, but actual operational presence
- **AI adoption signal vs. marketing claim** — companies that say "AI-powered" in their pitch vs. companies whose engineering teams, products, and hiring patterns confirm actual AI integration

---

## A Sample Profile from the Ritual

```
Company:            EnergyTech BV
Country:            Netherlands
Classification:     Rocket
Growth Score:       8.2 / 10
Financial Health:   7.4 / 10
Competitive Pos.:   8.0 / 10

Evidence Signals:
  ✓ Revenue CAGR 34% (3-year average confirmed via multiple signals)
  ✓ Series B — EUR 22M raised, Q3 2024
  ✓ AI-native product architecture (not bolt-on — confirmed in engineering stack)
  ✓ ESMIG protocol certified (strong European grid compliance position)
  ✗ No ENTSO-E mapping yet — one certified competitor has this, gap identified

Competitive Overlap: 6 direct competitors identified and ranked by threat level
Transformation potential: N/A — already a Rocket. Watch for acquisition premium.
```

---

## The Comparison Spell

| | Old Ritual | Solstein |
|---|---|---|
| Price | EUR 500K–1.5M | EUR 60K pilot |
| Time | 90 days | 3 days |
| Companies covered | 5–10 | 29 |
| Freshness | Static on delivery | Continuously refreshable |
| Reasoning visible | Never | Always |
| Protocol mapping | Sometimes, high-level | Complete — every company, every protocol |
| Transformation signal | Not included | Built into every Dinosaur classification |

---

## What This Intelligence Actually Opens

**1. Target screening before committing to full DD**
Rank the 29 companies in 3 days. Spend DD budget on the 3 that matter. Skip the 7 that are irreversible Dinosaurs.

**2. Competitive gap analysis for a target acquisition**
Before acquiring Company X, see exactly where its 6 direct competitors are stronger, weaker, and on what dimensions.

**3. Synergy validation — the real kind**
The deal deck claims protocol overlap with the target company's market. Solstein maps the actual protocol certifications of every player. The claimed synergies either exist or they do not.

**4. Exit preparation**
For a portfolio company approaching exit, Solstein's Attractiveness Board shows the acquirer exactly how this company scores against the full market — and why the premium is justified.

---

## The Next Markets We Can Open

The spell is not limited to one market. Every structured market with identifiable players is a candidate.

- DACH energy software (Germany, Austria, Switzerland)
- European building automation and smart building platforms
- Nordic utilities management software
- Any SaaS vertical where competitive clarity changes deal outcomes

---

## What We Invite You to Do

Let us walk you through these 29 companies. In the session, you will see:

- How the scores were produced and what evidence underlies each
- How the Dinosaur classifications were made — and which of the 7 have transformation potential
- How the protocol map exposes gaps that standard DD misses

**Duration: 45 minutes. No slide deck. Live data only.**

---

---
*A demonstrative cast by [AI Whisperers](https://www.ai-whisperers.org/).*

---
