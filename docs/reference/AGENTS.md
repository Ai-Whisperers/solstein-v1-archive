...
```markdown
□ No functions exceed 100 lines (target: <50)
□ No classes exceed 300 lines or 15 methods (target: <200)
□ No files exceed 500 lines (target: <400)
...
□ New code does not increase smell count
```
...
python scripts/ci/code_smell_detector.py <your-file.py>
python scripts/ci/check_function_sizes.py <your-file.py>
python scripts/ci/check_class_sizes.py <your-file.py>

# Or run all checks
python scripts/ci/quality_check.py --only-required

# Check PR size (if preparing PR)
python scripts/ci/check_pr_size.py
```
...
1. **P0 (Blockers)**: Function >100 lines, Class >300 lines, File >500 lines
   - Extract methods/classes immediately
   - Use Strategy Pattern for complex logic

2. **P0 (Blockers)**: Bare except clauses
   - Replace with specific exception types
   - Add proper error logging

3. **P1 (Warnings)**: Function >50 lines, Deep nesting (>4 levels)
   - Refactor to improve readability
   - Use guard clauses and early returns

4. **P2 (Suggestions)**: Many parameters (>5), Complex conditionals
