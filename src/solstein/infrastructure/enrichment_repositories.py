...
class EnrichmentAuditRepository:
    ...
    async def get_company_stats(self, company_id: str) -> dict:
        ...
        query = select(EnrichmentAuditRecord).where(
            EnrichmentAuditRecord.company_id == company_id
        )
        result = await self.session.execute(query)
...
class EnrichmentCacheRepository:
    ...
    async def get_cached(self, company_id: str) -> Optional[EnrichmentCacheRecord]:
        """Get cached enrichment data for a company."""
        query = select(EnrichmentCacheRecord).where(
    ...
    async def delete_cache(self, company_id: Optional[str] = None) -> int:
        """Delete cache entries. If company_id is None, delete all expired entries."""
        if company_id:
            query = delete(EnrichmentCacheRecord).where(
                EnrichmentCacheRecord.company_id == company_id
            )
        else:
            ...
            now = datetime.now(timezone.utc)
            query = delete(EnrichmentCacheRecord).where(
                EnrichmentCacheRecord.expires_at <= now
            )
