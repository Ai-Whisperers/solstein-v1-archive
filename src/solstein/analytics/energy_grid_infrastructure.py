...
_GRID_CONNECTIVITY_KEYWORDS = frozenset({
    "der management", "distributed energy resource", "grid-edge",
    "grid edge", "v2g", "vehicle-to-grid", "grid integration",
    "frequency regulation", "voltage regulation", "power quality",
    "grid stability", "ancillary services",
})

_DER_KEYWORDS = frozenset({
    "solar inverter", "battery inverter", "charge controller",
    "power electronics", "grid-tied", "islanding",
    "net metering", "feed-in", "curtailment",
})

_SMART_INFRA_KEYWORDS = frozenset({
    "iot", "sensor", "smart meter", "smart sensor",
    "edge computing", "fog computing", "embedded",
    "telemetry", "remote monitoring", "condition monitoring",
})
...
class GridInfrastructureScorer:
    ...
    def score(self, company: Company) -> GridInfrastructureResult:
        ...
        result = GridInfrastructureResult(
            ...
            breakdown={
                ...
                "grid_signals": sum(
                    1 for s in signals if s.category == "grid"
                ),
                "smart_signals": sum(
                    1 for s in signals if s.category == "smart"
                ),
                "decentralization_signals": sum(
                    1 for s in signals if s.category == "decentralization"
                ),
            },
        )
        logger.info(
            ...
            grid_ready.value, smart_level.value,
        )
        return result
    ...
    def _score_grid_connectivity(
        ...
        if grid_count >= 3:
            ...
            signals.append(GridSignal(
                "grid", "Advanced Grid Integration", "advanced",
                f"Detected {grid_count} grid connectivity capabilities",
            ))
        elif grid_count >= 1:
            score += 12.0
            signals.append(GridSignal(
                "grid", "Basic Grid Integration", "developing",
        ...
        if vpp_count >= 2:
            score += 20.0
            signals.append(GridSignal(
                "grid", "Virtual Power Plant", "advanced",
    ...
    def _score_smart_infrastructure(
        ...
        if iot_count >= 3:
            score += 25.0
            signals.append(GridSignal(
                "smart", "IoT Infrastructure", "advanced",
                f"Strong IoT/sensor layer ({iot_count} indicators)",
            ))
        elif iot_count >= 1:
            score += 12.0
            signals.append(GridSignal(
                "smart", "Basic Monitoring", "developing",
        ...
        if twin_count >= 2:
            score += 20.0
            signals.append(GridSignal(
                "smart", "Digital Twin / Predictive", "advanced",
    ...
    def _score_decentralization(
        ...
        if micro_count >= 2:
            score += 25.0
            signals.append(GridSignal(
                "decentralization", "Microgrid Capability", "advanced",
                f"Microgrid capabilities ({micro_count} indicators)",
            ))
        elif micro_count >= 1:
            score += 12.0
            signals.append(GridSignal(
                "decentralization", "Microgrid Interest", "developing",
        ...
        if p2p_count >= 2:
            score += 25.0
            signals.append(GridSignal(
                "decentralization", "P2P Energy Trading", "advanced",
...
def _classify_grid_readiness(
    grid_score: float, decentralization: float,
) -> GridReadiness:
...
def _identify_capabilities(
    grid: float, smart: float, decentralization: float,
) -> list[str]:
...
def _build_recommendations(
    ...
    for gap in gaps:
        if "grid connectivity" in gap.lower():
            recs.append(
                "Develop DER management and grid-edge intelligence capabilities"
            )
        elif "traditional infrastructure" in gap.lower():
            recs.append(
                "Invest in IoT sensor layer and predictive maintenance; "
                "explore digital twin technology"
            )
        elif "decentralization" in gap.lower():
            recs.append(
                "Explore microgrid and virtual power plant solutions "
                "for decentralized energy management"
            )

    if not recs and grid_ready != GridReadiness.ADVANCED:
        recs.append(
            "Continue grid modernization; explore V2G and advanced "
            "DER aggregation"
        )
    if not recs and smart_level != SmartInfraLevel.INTELLIGENT:
        recs.append(
            "Evaluate AI-driven grid optimization and digital twin "
            "deployment"
        )
    return recs
