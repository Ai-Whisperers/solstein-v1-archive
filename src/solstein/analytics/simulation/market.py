...
class SimulationEngine:
    ...
    def run(
        self, scenario: Scenario, companies: list[Company]
        ...
        """
        logger.info(
            f"Running simulation '{scenario.name}' on {len(companies)} companies"
        )
        results = []
    ...
    def _simulate_company(
        self, scenario: Scenario, company: Company
    ) -> SimulationResult:
        ...
        for condition in scenario.conditions:
            ...
            if condition.affected_industries:
                if not company.industry:
                    continue
                if not any(
                    ind.lower() in company.industry.lower()
                    for ind in condition.affected_industries
                ):
                    continue
            ...
            match condition.type:
                case MarketConditionType.INTEREST_RATE:
                    ...
                    if condition.impact_factor > 1.0:
                        drop_pct = (
                            condition.impact_factor - 1.0
                        ) * 2.0  # Magnified impact
                        simulated_valuation *= 1.0 - drop_pct
                        notes.append(
                            f"Valuation impacted by interest rates (-{drop_pct:.1%})"
                        )
                    else:
                        ...
                        notes.append(
                            f"Valuation boosted by low interest rates "
                            f"(+{boost_pct:.1%})"
                        )
                ...
                case MarketConditionType.SECTOR_GROWTH:
                    ...
                    notes.append(
                        f"Growth score adjusted by sector trend "
                        f"({condition.impact_factor:+})"
                    )
                ...
                case MarketConditionType.COMPETITOR_ACTIVITY:
                    ...
                    if condition.impact_factor > 0:
                        ...
                        notes.append(
                            f"Growth score reduced by competition "
                            f"(-{condition.impact_factor * 0.5})"
                        )
                ...
                case MarketConditionType.INFLATION:
                    ...
                    if condition.impact_factor > 0.02:  # Normal inflation < 2%
                        ...
                        notes.append(
                            f"Valuation reduced by high inflation (-{impact:.1%})"
                        )
