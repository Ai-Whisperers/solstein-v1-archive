...
class CompanyValidator:
    ...
    def _validate_revenue(cls, data: dict[str, Any]) -> list[ValidationIssue]:
        ...
        if not timeline:
            issues.append(
                ValidationIssue(
                    field="revenue.timeline", message="Revenue timeline is missing", severity=ValidationSeverity.ERROR
                )
            )
            return issues
        ...
        if revenue is None:
            issues.append(
                ValidationIssue(
                    field="revenue.eur_millions", message="Revenue value is missing", severity=ValidationSeverity.ERROR
                )
            )
        elif revenue < 0:
            issues.append(
    ...
    def _validate_employees(cls, data: dict[str, Any]) -> list[ValidationIssue]:
        ...
        if employees is None:
            issues.append(
                ValidationIssue(
                    field="employees", message="Employee count is missing", severity=ValidationSeverity.WARNING
                )
            )
        elif employees < 0:
            issues.append(
