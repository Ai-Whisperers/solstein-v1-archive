...
_NO_API_KEYWORDS = frozenset({
    "no api", "no rest", "manual", "spreadsheet-driven",
})
...
def _check_ai_readiness(company: Company) -> list[RedFlag]:
    ...
    if ai_maturity in ("none", "low"):
        flags.append(RedFlag(
            ...
            description=(
                f"AI maturity: {ai_maturity}. "
                "Company has no AI initiatives, increasing transformation risk."
            ),
            severity=RedFlagSeverity.HIGH,
    ...
    if growth_rate is not None and growth_rate < 0:
        flags.append(RedFlag(
            ...
            description=(
                ...
                "Declining revenue may limit AI investment capacity."
            ),
            severity=RedFlagSeverity.HIGH,
    ...
    if employees is not None and employees < 30:
        flags.append(RedFlag(
            ...
            description=(
                ...
                "Team may lack capacity for parallel AI initiatives."
            ),
            severity=RedFlagSeverity.HIGH,
