...
class FinancialMetric(BaseModel):
    ...
    revenue_confidence: ConfidenceLevel = Field(
        default=ConfidenceLevel.UNKNOWN, description="Confidence level for revenue data"
    )
    ...
    growth_confidence: ConfidenceLevel = Field(default=ConfidenceLevel.UNKNOWN)

    employees: int | None = Field(
        default=None, description="Number of employees", ge=0, examples=[50, 1000, 10000]
    )
    employees_confidence: ConfidenceLevel = Field(default=ConfidenceLevel.UNKNOWN)
    ...
    margin_confidence: ConfidenceLevel = Field(default=ConfidenceLevel.UNKNOWN)
    ...
    funding_confidence: ConfidenceLevel = Field(default=ConfidenceLevel.UNKNOWN)
    ...
    valuation_confidence: ConfidenceLevel = Field(default=ConfidenceLevel.UNKNOWN)
...
class CompanyProfile(BaseModel):
    ...
    founded_year: int | None = Field(
        default=None, description="Year company was founded", ge=1800, le=datetime.now().year
    )

    # Competitive positioning
    tier: CompanyTier = Field(
        default=CompanyTier.TIER_3, description="Market position tier"
    )
    threat_level: ThreatLevel = Field(
        default=ThreatLevel.MEDIUM, description="Competitive threat level"
    )

    # Technology assessment
    ai_maturity: AIMaturity = Field(
        default=AIMaturity.NONE, description="AI adoption maturity level"
    )
    saas_maturity: int = Field(
        default=1, description="SaaS maturity score (1-10)", ge=1, le=10
    )
    tech_stack: list[str] = Field(default_factory=list, description="Technology stack")

    # Financial data
    financials: FinancialMetric = Field(default_factory=lambda: FinancialMetric())

    # Market presence
    geographic_presence: list[str] = Field(
        default_factory=list, description="Countries/regions where company operates"
    )
    key_customers: list[str] = Field(
        default_factory=list, description="Notable customers"
    )

    # Corporate structure
    parent_company: str | None = Field(default=None, description="Parent company if subsidiary")
    ...
    last_updated: datetime = Field(
        default_factory=datetime.now, description="When this profile was last updated"
    )
    data_source: str | None = Field(default=None, description="Source of this data")
    notes: str | None = Field(default=None, description="Additional notes")

    # Calculated fields (will be populated by analytics)
    growth_score: float | None = Field(
        default=None, description="Overall growth score (0-10)", ge=0, le=10
    )
    financial_health_score: float | None = Field(
        default=None, description="Financial health score (0-10)", ge=0, le=10
    )
    competitive_position_score: float | None = Field(
        default=None, description="Competitive position score (0-10)", ge=0, le=10
    )
...
class MarketAnalysis(BaseModel):
    ...
    analysis_date: datetime = Field(
        default_factory=datetime.now, description="When this analysis was conducted"
    )

    companies: list[CompanyProfile] = Field(
        default_factory=list, description="Companies in this market"
    )

    # Market metrics
    total_market_size: float | None = Field(
        default=None, description="Total market size in EUR", ge=0
    )
    growth_rate: float | None = Field(
        default=None, description="Market growth rate in percentage", ge=-100, le=1000
    )

    # Competitive landscape
    concentration_ratio: float | None = Field(
        default=None, description="Market concentration ratio (CR4)", ge=0, le=100
    )
    barriers_to_entry: list[str] = Field(
        default_factory=list, description="Barriers to entry in this market"
    )

    # Trends
    key_trends: list[str] = Field(default_factory=list, description="Key market trends")
    regulatory_environment: list[str] = Field(
        default_factory=list, description="Regulatory factors"
    )

    # Analysis
    swot_analysis: dict[str, list[str]] | None = Field(
        default=None, description="SWOT analysis for the market"
    )
...
class CompetitiveOverlap(BaseModel):
    ...
    overlap_score: float = Field(..., description="Overlap score (0-1)", ge=0, le=1)

    overlap_areas: list[str] = Field(
        default_factory=list, description="Areas of overlap"
    )
    ...
    notes: str | None = Field(
        None, description="Additional notes on the competitive relationship"
    )
