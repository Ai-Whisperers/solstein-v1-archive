...
from ..domain.models import Company, ConfidenceLevel, FinancialMetric
...
class UnifiedCompanyLoader:
    ...
    def __init__(
        self,
        sec_connector: SECEdgarConnector | None = None,
        companies_house_connector: CompaniesHouseConnector | None = None,
        news_detector: NewsSignalDetector | None = None,
    ):
    ...
    def enrich_batch(self, companies: list[UnifiedCompany], batch_size: int = 10) -> list[UnifiedCompany]:
        ...
        logger.info(
            f"✅ Starting batch enrichment: {len(companies)} companies in {total_batches} batches of {batch_size}"
        )
    ...
    def fill_nulls_from_companies_house(self, company: UnifiedCompany) -> UnifiedCompany:
        ...
        """
        logger.info(
            f"[CONNECTOR-CH-START] Companies House enrichment STARTED for {company.name} (company_number={company.company_number})"
        )
        ...
        try:
            ch_data = self.companies_house_connector.get_company_metrics(company_number=company.company_number.strip())
            ...
            if ch_data.get("revenue") is not None:
                ...
                if is_valid_number(revenue_gbp) and 0 < revenue_gbp < 1e15:
                    ...
                    if company.financials.revenue is None:
                        ...
                        logger.debug(
                            f"Filled revenue for {company.name} from Companies House: {revenue_eur} EUR (from {revenue_gbp} GBP)"
                        )
            ...
            logger.info(
                f"[CONNECTOR-CH-END] Companies House enrichment COMPLETED for {company.name}. Revenue: {company.financials.revenue}"
            )
            return company
