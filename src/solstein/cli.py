...
from .extractors.markdown_extractor import BatchExtractor, MarkdownExtractor
...
def extract(input_dir: Path, output: Optional[Path], pattern: str):
...
def export_excel(input_file: Path, output_file: Path, template: Optional[Path]):
...
def score(input_file: Path, output: Optional[Path]):
