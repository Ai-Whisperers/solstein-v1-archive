...
class UsageTracker:
    ...
    def record_usage(
        ...
        cost_per_1k = MODEL_COSTS.get(model, {"input": 0.0, "output": 0.0})
        ...
        self.costs_by_provider[provider] = self.costs_by_provider.get(provider, 0.0) + total_cost

        logger.info(
            f"LLM usage: {provider}/{model} - "
            f"{input_tokens}in/{output_tokens}out tokens, ${total_cost:.4f}",
            provider=provider,
            model=model,
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "costs_by_provider": {k: round(v, 4) for k, v in self.costs_by_provider.items()},
        }
