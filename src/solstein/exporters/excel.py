    ...
    def _add_financial_intelligence(self, ws: Any, profiles: list[Company]) -> None:
        ...
        for i, p in enumerate(profiles, 5):
            ...
            data = [
                ...
                f"{p.financials.profit_margin:.1f}%" if p.financials and p.financials.profit_margin is not None else "N/A",
                f"{p.financials.ebitda_margin:.1f}%" if p.financials and p.financials.ebitda_margin is not None else "N/A",
