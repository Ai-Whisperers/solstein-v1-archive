    ...
    def get_row_fill(self, row_idx: int, classification: str | None = None) -> PatternFill:
        ...
        if classification:
            if classification == "Phoenix":
                return PatternFill(
                    start_color=self.colors.PHOENIX_FILL, end_color=self.colors.PHOENIX_FILL, fill_type="solid"
                )
            elif classification == "Lead":
                return PatternFill(
                    start_color=self.colors.LEAD_FILL, end_color=self.colors.LEAD_FILL, fill_type="solid"
                )
            elif classification == "Salt":
                return PatternFill(
                    start_color=self.colors.SALT_FILL, end_color=self.colors.SALT_FILL, fill_type="solid"
                )
...
class ImprovedExcelExporter:
    ...
    def _write_headers(self, ws: Any, headers: list[str], row: int | None = None) -> None:
    ...
    def _format_number(self, value: float | None, decimals: int = 1, suffix: str = "") -> str:
    ...
    def _format_percentage(self, value: float | None, decimals: int = 1) -> str:
    ...
    def _add_executive_summary(self, ws: Any, profiles: list[Company]) -> None:
        ...
        summary_data = [
            ...
            [
                "Phoenix Companies",
                f"{phoenix_count} ({self._format_percentage(phoenix_count / total * 100 if total else 0, 0)})",
            ],
            [
                "Salt Companies",
                f"{salt_count} ({self._format_percentage(salt_count / total * 100 if total else 0, 0)})",
            ],
            [
                "Lead Companies",
                f"{lead_count} ({self._format_percentage(lead_count / total * 100 if total else 0, 0)})",
            ],
            ["Average Composite Score", self._format_number(avg_score, 2)],
            ["Score Range", f"{min_score:.2f} – {max_score:.2f}" if valid_scores else "N/A"],
            [
                "Synthetic Companies",
                f"{synthetic_cnt} ({self._format_percentage(synthetic_cnt / total * 100 if total else 0, 0)})",
            ],
    ...
    def _add_market_rankings(self, ws: Any, profiles: list[Company]) -> None:
        ...
        sorted_profiles = sorted(profiles, key=lambda p: getattr(p, "composite_score", 0) or 0, reverse=True)
    ...
    def _add_financial_intelligence(self, ws: Any, profiles: list[Company]) -> None:
        ...
        for idx, profile in enumerate(profiles):
            ...
            growth_conf_str = (
                growth_conf.value if hasattr(growth_conf, "value") else str(growth_conf) if growth_conf else "Unknown"
            )
            ...
            fund_conf_str = (
                fund_conf.value if hasattr(fund_conf, "value") else str(fund_conf) if fund_conf else "Unknown"
            )
    ...
    def _add_tech_ai_maturity(self, ws: Any, profiles: list[Company]) -> None:
        ...
        headers = ["Company", "AI Maturity", "SaaS Maturity", "Tech Stack", "AI in Production", "Key Capabilities"]
        self._write_headers(ws, headers)
    ...
    def _auto_adjust_columns(self, ws: Any) -> None:
        ...
        for column in ws.columns:
            ...
            adjusted_width = min(max(max_length + 2, self.layout.MIN_COLUMN_WIDTH), self.layout.MAX_COLUMN_WIDTH)
