...
competitors = [
    ...
    ("SOPTIM", "SOPTIM AG", {"DE", "AT"}, {"BO"}, {"RET", "GRD"}, 1),
    ("Trayport", "Trayport (TMX Group)", {"UK", "NL"}, {"ET"}, {"TRD", "RET"}, 1),
    (
        "Brady",
        "Brady Technologies",
        {"UK", "NO", "PL", "US"},
    ...
    ("KISTERS", "KISTERS AG", {"DE", "AT", "NL"}, {"BO"}, {"RET", "GRD"}, 1),
    (
        "SopraS",
    ...
    ("Hansen", "Hansen Technologies", {"NL", "UK", "DE", "NO"}, {"BI"}, {"RET"}, 1),
    ("Robotron", "Robotron Datenbank-Software GmbH", {"DE"}, {"BI"}, {"RET"}, 2),
    ("EG", "EG A/S (Utility Division)", {"NO"}, {"BI"}, {"RET"}, 2),
    ("Tietoevry", "Tietoevry (Tieto)", {"NO"}, {"BI", "CO"}, {"RET"}, 2),
    ("MaxBill", "MaxBill (LogNet Systems)", {"UK", "NL", "PL"}, {"BI"}, {"RET"}, 2),
    (
        "Indra",
        "Indra Sistemas / Minsait",
        {"ES", "IT"},
        {"BO", "PL"},
        {"RET", "GRD"},
        2,
    ),
    ("EngGrp", "Engineering Group", {"IT", "ES", "DE", "BE"}, {"BI"}, {"RET"}, 2),
    (
        "Ferranti",
    ...
    ("Asseco", "Asseco Poland S.A.", {"PL", "EE"}, {"BI"}, {"RET"}, 2),
    ("ION", "ION Commodities (ION Group)", {"UK", "US"}, {"ET"}, {"TRD"}, 3),
    ("Hitachi", "Hitachi Energy", {"CH", "US", "DE"}, {"ET"}, {"RET", "TRD", "GRD"}, 3),
    ("EnergyOne", "Energy One Limited", {"UK", "FR", "BE"}, {"ET"}, {"RET", "TRD"}, 3),
    (
        "Previse",
        "Previse Systems AG",
        {"CH", "DE", "NL", "UK", "NO", "ES", "IT", "PL", "BE"},
        {"ET"},
        {"TRD", "RET"},
        3,
    ),
    ...
    ("Molecule", "Molecule Software", {"US", "UK"}, {"ET"}, {"TRD"}, 3),
    ("Qualia", "Qualia Trading", {"UK"}, {"ET"}, {"TRD"}, 3),
    (
        "Octopus",
    ...
    ("tem", "tem (tem energy)", {"UK"}, {"PL"}, {"IND"}, 4),
    (
        "Dexter",
        "Dexter Energy",
        {"NL", "DE", "BE", "FR", "UK", "IT"},
    ...
    ("ALTEN", "ALTEN Worldgrid", {"FR", "ES", "DE"}, {"CO"}, {"RET", "GRD"}, 4),
    ("CGI", "CGI Inc.", {"NL", "FR", "UK"}, {"PL", "IN"}, {"GRD", "INF"}, 4),
    ("SEEBURGER", "SEEBURGER AG", {"DE"}, {"IN"}, {"INF"}, 4),
    (
        "Arvato",
    ...
    ("Schleupen", "Schleupen SE", {"DE"}, {"BI", "IN"}, {"RET"}, 4),
    ("Creatica", "Creatica GmbH", {"DE"}, {"ET"}, {"TRD", "GEN"}, 4),
]
...
for j in range(1, N):
    ...
    lines.append(
        f"  {c[0]:12s}: score={matrix[0][j]}  [geo={g} prod={p} cust={cu} tier={t}]"
    )
...
for i in range(N):
    for j in range(i + 1, N):
        ...
        raw_pairs.append(
            (raw, geo_cnt + prod_cnt + cust_cnt, competitors[i][0], competitors[j][0])
        )
