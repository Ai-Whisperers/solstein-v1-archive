...
""")
                for i, ap in enumerate(god_patterns[:10], 1):
                    f.write(
                        f"| STORY-{epic_num}.{i} | Refactor {ap.name} in {ap.location} | 5 | P{0 if ap.severity == 'critical' else 1} |\n"
                    )
...
""")
        ...
        quality_patterns = [
            ap for ap in self.anti_patterns if ap.category == "quality" and ap.severity in ["high", "critical"]
        ]
        if quality_patterns:
            epic_num = len(epics) + 51
