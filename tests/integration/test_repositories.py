...
async def sample_company(db_session: AsyncSession) -> CompanyRecord:
    ...
    company = CompanyRecord(id="test-company", ticker="TEST", name="Test Company", status="active", sector="Technology")
...
class TestCompanyRepository:
    ...
    async def test_create_company(self, company_repo: CompanyRepository):
        ...
        company = await company_repo.create(
            ticker="NEW", name="New Company", sector="Finance", metadata={"employees": 500}
        )
    ...
    async def test_get_all(self, company_repo: CompanyRepository, db_session: AsyncSession):
        ...
        for i in range(5):
            company = CompanyRecord(id=f"company-{i}", ticker=f"TICK{i}", name=f"Company {i}", status="active")
            db_session.add(company)
        await db_session.flush()
    ...
    async def test_update(self, company_repo: CompanyRepository, sample_company: CompanyRecord):
        ...
        updated = await company_repo.update(sample_company.id, name="Updated Company", status="inactive")
    ...
    async def test_delete(self, company_repo: CompanyRepository, db_session: AsyncSession):
        ...
        company = CompanyRecord(id="delete-test", ticker="DEL", name="Delete Test", status="active")
        db_session.add(company)
        await db_session.flush()
    ...
    async def test_update_metadata(self, company_repo: CompanyRepository, sample_company: CompanyRecord):
        ...
        updated = await company_repo.update_metadata(sample_company.id, {"revenue": 1000000, "employees": 100})
...
class TestFactRepository:
    ...
    async def test_get_facts_by_company(
        self, fact_repo: FactRepository, sample_company: CompanyRecord, db_session: AsyncSession
    ):
    ...
    async def test_get_facts_by_company_with_status_filter(
        ...
        active_fact = FactRecord(
            id="fact-active", company_id=sample_company.id, fact_key="active_metric", fact_value="100", status="active"
        )
        superseded_fact = FactRecord(
            id="fact-super", company_id=sample_company.id, fact_key="old_metric", fact_value="50", status="superseded"
        )
    ...
    async def test_update_fact_confidence(
        ...
        fact = FactRecord(
            id="fact-update", company_id=sample_company.id, fact_key="test_metric", fact_value="100", confidence=0.5
        )
        db_session.add(fact)
        await db_session.flush()
    ...
    async def test_supersede_fact(
        ...
        fact = FactRecord(
            id="fact-super-1", company_id=sample_company.id, fact_key="revenue", fact_value="1000000", status="active"
        )
        db_session.add(fact)
        await db_session.flush()
...
class TestRepositoryTransactions:
    ...
    async def test_atomic_operations(self, db_session: AsyncSession):
        ...
        # Create company
        company = await company_repo.create(ticker="ATOMIC", name="Atomic Test", status="active")
    ...
    async def test_cascading_operations(self, db_session: AsyncSession):
        ...
        company = CompanyRecord(id="cascade-test", ticker="CASC", name="Cascade Test", status="active")
        db_session.add(company)
        await db_session.flush()
        ...
        for i in range(3):
            await fact_repo.create(company_id=company.id, fact_key=f"metric_{i}", fact_value=str(i * 100))
...
class TestRepositoryPerformance:
    ...
    async def test_bulk_create_performance(self, db_session: AsyncSession):
        ...
        start = time.time()
        for i in range(100):
            await company_repo.create(ticker=f"BULK{i:03d}", name=f"Bulk Company {i}", status="active")
    ...
    async def test_query_with_pagination(self, company_repo: CompanyRepository, db_session: AsyncSession):
        ...
        for i in range(50):
            company = CompanyRecord(
                id=f"page-company-{i}", ticker=f"PAGE{i:02d}", name=f"Page Company {i}", status="active"
            )
            db_session.add(company)
        await db_session.flush()
