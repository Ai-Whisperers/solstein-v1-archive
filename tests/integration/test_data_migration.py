...
from src.solstein.infrastructure.models import (
    ...
    ContradictionRecord, OutboxRecord, ScoringRecord, SignalRecord as SignalRecordModel,
    EnrichmentJobRecord, AuditTrailRecord, MarketSnapshotRecord
)
...
class TestDataIntegrity:
    ...
    async def test_required_fields_not_null(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(
            text("SELECT COUNT(*) FROM companies WHERE ticker IS NULL OR ticker = ''")
        )
        null_tickers = result.scalar()
        assert null_tickers == 0, f"Found {null_tickers} companies with null/empty ticker"
        
        result = await db_session.execute(
            text("SELECT COUNT(*) FROM companies WHERE status IS NULL")
        )
