...
from solstein.domain.models import Company, FinancialMetric, AIMaturity, ThreatLevel, CompanyTier

def make_company(**kwargs):
    fin = FinancialMetric()
    base = {
        "id": "c1", "name": "C1", "description": "desc",
        "industry": "Tech", "financials": fin,
        "ai_maturity": AIMaturity.NONE, "threat_level": ThreatLevel.MEDIUM,
        "tier": CompanyTier.TIER_3, "geographic_presence": [], "tech_stack": [],
        "key_customers": []
    }
    base.update(kwargs)
    return Company(**base)
...
def test_growth_scorer_fallback():
    ...
    with patch.object(scorer, "_calculate_financial_health_score", return_value=(None, None)):
...
def test_competitive_overlap_customer_overlap():
    ...
    assert calc._calculate_geographic_overlap(make_company(geographic_presence=["US"]), make_company(geographic_presence=[])) == 0.0
