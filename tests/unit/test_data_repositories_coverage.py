...
def sample_companies():
    return [
        Company(id="1", name="Alpha", industry="Tech", financials=FinancialMetric(revenue=500.0), tier=CompanyTier.TIER_2, classification="Rocket"),
        Company(id="2", name="Beta", industry="Energy", financials=FinancialMetric(revenue=10.0), tier=CompanyTier.TIER_3, classification="Neutral"),
        Company(id="3", name="Gamma", industry="Tech Energy", financials=FinancialMetric(revenue=None), tier=CompanyTier.TIER_4, classification="Dinosaur"),
    ]
...
def test_supabase_repo_conversions(mock_get_sb):
    ...
    company.classification = "Rocket" # String
    # Also test the internal json_safe using a dict with datetime and enum
    company.scoring_breakdown = {"test": datetime(2023, 1, 1), "enum": DummyEnum.VAL, "list_nested": [{"dt": datetime(2023,1,1)}]}
