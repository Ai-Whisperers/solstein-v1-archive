...
from solstein.analytics.activities import _get_repo, calculate_company_score, fetch_market_company_ids
from solstein.analytics.simulation import SimulationEngine
from solstein.domain.models import Company, FinancialMetric
from solstein.domain.simulation import Scenario, MarketCondition, MarketConditionType
...
def test_get_repo_exception_fallback(mock_get_settings):
    ...
    with patch("solstein.analytics.activities.SupabaseRepository", side_effect=Exception("DB Error")):
        repo = _get_repo()
...
def test_simulation_engine_no_industry():
    ...
    company = Company(
        id="c1", name="C1", description="desc", industry="", # Missing industry / empty
