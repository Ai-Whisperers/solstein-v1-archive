...
from solstein import InterceptHandler
...
def test_company_repository_abstract_methods():
    class DummyRepo(CompanyRepository):
        def get_all(self, limit=None, offset=0, filters=None):
            return super().get_all(limit, offset, filters)
        ...
        def search(self, query, field="name"):
            return super().search(query, field)
